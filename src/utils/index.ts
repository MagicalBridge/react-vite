...
async function depositWithPermit(tokenAddress:string, bankAddress:string, amount:bigint, signer: ethers.JsonRpcSigner) {
  try {
    // 创建 token 合约实例
    ...
    // 获取链 ID
    const chainId = await signer.provider.getNetwork().then((network: ethers.Network) => network.chainId);
...
export async function main() {
  ...
  try {
    const txHash = await depositWithPermit(tokenAddress, bankAddress, amount, signer);
