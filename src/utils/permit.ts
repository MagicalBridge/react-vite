...
export default async function testPermitBuyNFT() {
  ...
  try {
    ...
    const value = {
      owner: '0x7437D6C9752A662B66944Dca31fAa25559dC8c78',
      buyer: address,
    };

    // 签名 permit 消息
    const permitWL = await signer.signTypedData(domain, types, value);

    const {
      v: permitWLV,
      r: permitWLR,
      s: permitWLS
    } = ethers.Signature.from(permitWL);

     // 编码  permitWL 数据
    const permitWLData = ethers.AbiCoder.defaultAbiCoder().encode(
      ['uint8', 'bytes32', 'bytes32'],
      [permitWLV, permitWLR, permitWLS]
    );
